...
| **Credential denylist** | `~\.ssh`, `~\.aws`, `~\.gnupg`, `~\.gpg`, `~\.kube`, `~\.docker`, `~\.netrc`, `~\.password-store`, `~\AppData\Roaming\gh`, `~\AppData\Roaming\gcloud`, `~\AppData\Local\gcloud`, `~\AppData\Roaming\Microsoft\Credentials`, `~\AppData\Local\Microsoft\Credentials` are refused for add/discover. |
...
| **Command execution** | "Run" and "Open Terminal" launch *your own* command in *your own* terminal (Windows Terminal, or a throwaway `.bat`); `subprocess` is always called with argument lists (never `shell=True`). |
...
This Windows port carries forward every fix above unchanged — only the OS
integration points (folder picker, terminal, editor launch, credential paths)
were ported; the security-relevant logic (host checks, path confinement,
denylist, code-only reads) is identical code.
