#!/usr/bin/env python3
"""Dashhy — нативное окно (pywebview / WebView2 на Windows).

Точка входа по умолчанию. Поднимает локальный HTTP-сервер (server.py) и
показывает дашборд в нативном окне. Окно запоминает размер и позицию между
запусками. На Windows pywebview рендерит через Microsoft Edge WebView2
(рантайм уже встроен в Windows 11).
"""
...
def main():
    ...
    # gui='edgechromium' is the default pywebview picks on win32 anyway;
    # named explicitly so a mixed Python env (e.g. one with pywin32 extras
    # that could nudge autodetection) doesn't silently fall back to MSHTML.
