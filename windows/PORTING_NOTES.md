...
| Credential denylist | `~/.ssh`, `~/.aws`, `~/.gnupg`, `~/.kube`, `~/.docker`, `~/.netrc`, `~/.password-store`, `~/.config/gh`, `~/.config/gcloud`, `~/Library/Keychains` | `~\.ssh`, `~\.aws`, `~\.gnupg`, `~\.kube`, `~\.docker`, `~\.netrc`, `~\.password-store`, `~\AppData\Roaming\GitHub CLI`, `~\AppData\Roaming\gcloud`, `~\AppData\Local\gcloud`, `~\AppData\Roaming\Microsoft\Credentials`, `~\AppData\Local\Microsoft\Credentials` |
